...
export default function JazzCashPage() {
  ...
  const salt = "ak70pl4o41";
  const ID = "MC990547";
  const password = "1eb40cl1j1";
  const rurl = "http://api.womenworld.com.pk/jazzWalletCallback_ww";
  ...
  useEffect(() => {
    fetch("http://api.womenworld.com.pk/api/get_head_enrichment.php")
      .then((res) => res.json())
      .then((data) => {
  ...
  const generateReqID = (msisdn: string) => {
    ...
    return `ReqIdwomenworldM${msisdn}${randomNumber}`;
  };
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      // Telenor prefixes: 0340–0349 (92340–92349)
      const networkPrefix = msisdn.slice(2, 5);
      if (/^34[0-9]$/.test(networkPrefix)) {
        error = "You're not eligible for this service right now";
        event = "validation_error";
        setApiError(error);
        return;
      }
  ...
  const renderPhoneInput = (compact = false) => (
    ...
    const renderSubscribeCard = (mobile = false) => (
      ...
      className={` w-full rounded-[12px] border border-[#C74B64] bg-[#F5F5F5] shadow-[0px_0px_18px_0px_#C74B64] ${
        mobile ? "px-6 py-5 mt-2" : "px-9 py-9"
      }`}
    >
      ...
      <button
        className={`mt-5 flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg font-bold text-white disabled:cursor-not-allowed disabled:opacity-70 ${
      ...
      <div
        className={`mt- flex items-center justify-center text-[#4a4a4e] ${
          mobile ? "gap-2 text-[11px]" : "gap-2.5 text-[13px]"
        }`}
      >
